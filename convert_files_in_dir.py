"""
Конвертировать все файлы формата PDF из одной папки в WORD файлы в другую папку.
"""
import os.path
import time
...
def wait_and_click_screenshot(screenshot_path: str,
    ...
    :param timeout: время, в течении которого мы ждем скриншот
    :param confidence: уровень уверенности в том, что это именно тот скриншот
    :return: координаты найденного скриншота.
...
def waiting_disappear_screenshot(screenshot: str,
                                 timeout: int = 10,
    ...
    :param timeout:
    :param confidence:
    :return: если скриншот исчез с экрана, то выходим из функции, возвращаем None
    """
    start_time = time.time()
    while time.time() - start_time < timeout:
        try:
...
##################################################################################
def launch_desktop_app(process: str,
                       icon_from_panel: str,
                       icon_from_desktop: str,
    ...
    :param process: Процесс, который проверяем запущен он или нет.
    :param icon_from_panel:
    :param icon_from_desktop:
    :param confidence:
    :return: None
    ...
    for proc in psutil.process_iter():
        ...
        if name == process:
            ...
            wait_and_click_screenshot(icon_from_panel, confidence=confidence)
            return None
    # Если нужный процесс не запущен, то запускаем по иконке с рабочего стола.
    wait_and_click_screenshot(icon_from_desktop, confidence=confidence)
