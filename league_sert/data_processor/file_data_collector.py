"""
Распарсить word файл - получить данные из файла.

    Задач - обработать word файл и получить из него нужные данные.
    Нужные данные становятся свойствами объекта класса MainDataGetter.
...
FILE = r'C:\Users\RIMinullin\Documents\протоколы\почти полное от обыденникова\ППК для оцифровки\Оригиналы_наилучшее возможное качество\word\47109 23.05.2024.docx'
FILE2 = r'C:\Users\RIMinullin\Documents\протоколы\почти полное от обыденникова\ППК для оцифровки\Оригиналы_наилучшее возможное качество\word\44433 29.05.2024.docx'
FILE3 = r'C:\Users\RIMinullin\Documents\протоколы\почти полное от обыденникова\ППК для оцифровки\Оригиналы_наилучшее возможное качество\word\50445 03.05.2024.docx'
...
class PatternsTypesOfTable(enum.Enum):
    ...
    PRODUCTION_CONTROL_MEASUREMENT: str = r'№ п/п\s?Место измерений'


def find_out_type_of_table(text_from_first_two_cells: str) -> PatternsTypesOfTable:
    """ Определить тип таблицы по содержанию первой строки. """
    for pattern in PatternsTypesOfTable:
    ...
    def extract_data_from_all_tables(self):
        ...
        for table_number, table_ in enumerate(self.all_tables_in_file):  # Цикл по таблицам.
            ...
            if len(table_.rows) > 1:  # Проверка на наличие в таблице более двух строк.
                ...
                if type_of_table in {PatternsTypesOfTable.MAIN.name,
                                     PatternsTypesOfTable.SAMPLE.name}:
                    self.process_two_columns_table()

                if type_of_table in {PatternsTypesOfTable.RESULTS.name,
                                     PatternsTypesOfTable.PRODUCTION_CONTROL_MEASUREMENT.name}:
                    self.process_more_then_two_cols()


def get_one_string_from_file(word_file: str) -> str:
    """ Прочитать word файл, вернуть одну большую строку. """
    text = docx2txt.process(word_file)
    return text


class PatternsText(enum.Enum):
    """ Паттерны для определения типа таблицы. """
...
def get_main_information(text) -> tuple[str, str]:
    """ Найти и вернуть номер и дату протокола. """
    substring_start = re.search(PatternsText.SUBSTRING_START.value, text).start()
    substring_end = (re.search(PatternsText.SUBSTRING_END.value,
                               text[substring_start:]).start() + substring_start)
    substring_with_number = re.sub(r'[\t\n\s]*', '', text[substring_start:substring_end])

    # Найти и вернуть номер и дату протокола.
    number_of_protocol = re.search(PatternsText.NUMBER.value, substring_with_number).group(1)
    match = re.search(PatternsText.DATE.value, substring_with_number)
    date_of_protocol = match.group(1) + ' ' + match.group(2) + ' ' + match.group(3)

    return number_of_protocol, date_of_protocol


class PattProdContr(enum.Enum):
...
def collect_prod_control_data(text) -> dict:
    ...
    производственного контроля. """
    substr_start = re.search(PattProdContr.START_SUBSTR.value, text).start()
    prod_control_text = text[substr_start:]
    number_of_protocol = re.search(PatternsText.NUMBER.value, prod_control_text).group(1)

    match = re.search(PattProdContr.DATE.value, prod_control_text)
    date_of_protocol = (match.group(1) + ' ' + match.group(2) + ' ' + match.group(3))

    match = re.search(PattProdContr.NUMB_AND_DATE_OF_MEASUR.value, prod_control_text)
    date_of_measurement = match.group(1)

    match = re.search(PattProdContr.PLACE_OF_MEASUR.value, prod_control_text)
    place_of_measurement = match.group(1).strip()

    inner_conclusion = re.search(PattProdContr.INNER_PART_OF_CONCLUSION.value,
...
if __name__ == '__main__':
    ...
    # print(type(data_getter.data_from_tables))
    print(data_getter.data_from_tables.keys())
    # print(data_getter.main_number)
    # print(data_getter.main_date)
    print(data_getter.data_from_tables[(1, 'SAMPLE')])
    print(data_getter.data_from_tables[(2, 'RESULTS')])
    # # print(data_getter.data_from_tables)
    # print(data_getter.prod_control_data)
