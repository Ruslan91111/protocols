...
def data1():
    return {'Номер протокола': '№13435/23-Д', 'Дата протокола': '27 декабря 2023',
            ...
            {'Токсичные элементы, мг/кг:': ('Токсичные элементы, мг/кг:',
             ...
             '- свинец': ('0,185±0,055', 'не более 0,35', True), '- мышьяк':
                 ('<0,05', 'не более 0,15', True),
             '- кадмий': ('0,005±0,002', 'не более 0,07', True), '- ртуть':
                 ('< 0,002', 'не более 0,015', True),
...
def data3():
    return {'Номер протокола': '№13509/23-Д', 'Дата протокола': '29 декабря 2023',
            ...
            {'Токсичные элементы, мг/кг:': ('Токсичные элементы, мг/кг:',
                                            'Токсичные элементы, мг/кг:', '-'),
             '- свинец': ('0,048±0,019', 'не более 0,35', True), '- мышьяк':
                 ('<0,05', 'не более 0,15', True),
             '- кадмий': ('0,011±0,004', 'не более 0,07', True), '- ртуть':
                 ('< 0,002', 'не более 0,015', True),
             'Пестициды, мг/кг:': ('Пестициды, мг/кг:', 'Пестициды, мг/кг:', '-'),
             '- ГХЦГ(а, Р, у - изомеры)': ('<0,01', 'не более 0,5', True),
             '- ДДТ и его метаболиты': ('<0,01', 'не более 0,02', True),
...
def test_data1(data1):
    ...
    assert word_parser.get_all_required_data_from_word_file() == data1


def test_data2(data2):
    word_parser = WordFileParser(FILE_2)
    assert word_parser.get_all_required_data_from_word_file() == data2


def test_data3(data3):
    word_parser = WordFileParser(FILE_3)
    assert word_parser.get_all_required_data_from_word_file() == data3
